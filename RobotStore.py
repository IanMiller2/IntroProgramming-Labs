class Product:
    ...
    def countStock(self, count):
        if count <= self.quantity:
            return True
        else:
           return False
    def calculatePrice(self, count):
        totalCost = count * self.price
        return totalCost
    def updateStock(self, count):
        self.quantity -= count
...
def main():
    ...
    while cash > 0:
        ...
        prodId = int(vals[0])
        count = int(vals[1])
        if products[prodId].countStock(count):
            if cash >= products[prodId].calculatePrice(count):
                products[prodId].updateStock(count)
